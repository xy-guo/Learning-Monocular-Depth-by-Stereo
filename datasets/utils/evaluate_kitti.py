...
def evaluate_command_line():
    ...
    if args.split == 'kitti':
        ...
        for t_id in range(num_samples):
            ...
            # print("focal length, baseline: {}/{}, mul: {}".format(focal_length, baseline, baseline * focal_length))
...
def evaluate_images(pred_disparities, test_files='./utils/filenames/eigen_test_files.txt', gt_path="/home/xyguo/dataset/kitti_png/"):
    # --split eigen
    # --garg_crop
    # --predicted_disp_path. /$1/disparities_pp.npy
    # --gt_path ~/dataset/kitti_png/
    args = parser.parse_args(["--split", "eigen", "--garg_crop", "--predicted_disp_path", None, "--gt_path", gt_path])
    # print("evaluate args: {}".format(args))
    ...
    if args.split == 'kitti':
        ...
        if isinstance(test_files, str):
            test_files = read_text_lines(test_files)
            num_samples = 697
            ...
            assert False
        assert pred_disparities.shape[0] == num_samples

        # get gt filenames, calib info, image sizes, image filenames, cam ids
        gt_files, gt_calib, im_sizes, im_files, cams = read_file_data(test_files, args.gt_path)
        ...
        assert False, "invalid split name, either kitti or eigen"
    ...
    abs_rel_mean = abs_rel.mean()
    sq_rel_mean = sq_rel.mean()
    rms_mean = rms.mean()
    log_rms_mean = log_rms.mean()
    d1_all_mean = d1_all.mean()
    a1_mean = a1.mean()
    a2_mean = a2.mean()
    a3_mean = a3.mean()
    # print("tested {} images".format(num_samples))
    # print("{:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10}, {:>10}".format('num', 'abs_rel', 'sq_rel', 'rms', 'log_rms',
    #                                                                               'd1_all', 'a1', 'a2', 'a3'))
    # print("{:10d}, {:10.4f}, {:10.4f}, {:10.3f}, {:10.3f}, {:10.3f}, {:10.3f}, {:10.3f}, {:10.3f}".format(num_samples, abs_rel_mean,
    #                                                                                               sq_rel_mean,
    #                                                                                               rms_mean,
    #                                                                                               log_rms_mean,
    #                                                                                               d1_all_mean,
    #                                                                                               a1_mean, a2_mean,
    #                                                                                               a3_mean))
    return {"abs_rel": abs_rel_mean, "sq_rel": sq_rel_mean, "rms": rms_mean, "log_rms": log_rms_mean,
            "d1_all": d1_all_mean, "a1": a1_mean, "a2": a2_mean, "a3": a3_mean}


def evaluate_images_abs(pred_disparities, test_files='./utils/filenames/eigen_test_files.txt', gt_path="/home/xyguo/dataset/kitti_raw/"):
    assert len(pred_disparities.shape) == 3
    return evaluate_images(pred_disparities / pred_disparities.shape[2], test_files, gt_path)


def test_evaluate_image():
    args = parser.parse_args()
    pred_disparities = np.load(args.predicted_disp_path)
    evaluate_images(pred_disparities)
