...
from models import MonocularModel, StereoModel
...
parser.add_argument('--dataset', type=str, help='dataset', default="kitti", choices=["kitti", "cityscapes"])
...
parser.add_argument('--lrepochs', type=str, help='epoch ids when descending the learning rate', default="20,35,45")
...
parser.add_argument('--stereo_ckpt', type=str, help='the stereo model')
...
def train():
    ...
    # model
    model = DataParallelOnlyGatherFirst(MonocularModel()).cuda()
    ...
    # stereo model
    stereo_model = DataParallelOnlyGatherFirst(StereoModel()).cuda()
    stereo_model.eval()
    print("loading stereo checkpoint: {}".format(args.stereo_ckpt))
    stereo_model.load_state_dict(torch.load(args.stereo_ckpt)['model'])
    print("loaded")

    train_dataset = DistillDataset(args.data_path, args.train_list, args, args.dataset, 'train')
    val_dataset = DistillDataset(args.data_path, args.val_list, args, args.dataset, 'val')
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_threads, drop_last=True)
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_threads, drop_last=False)
    ...
    for epoch_idx in range(starting_epoch, args.num_epochs):
        ...
        for batch_idx, sample in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            stereo_disp_est = stereo_model(to_cuda_vars({"left": sample["stereo_left"], "right": sample["stereo_right"]}))[0].detach() / 1280.
            sample["left_stereo_disp_est"] = F.interpolate(stereo_disp_est, size=(args.height, args.width), mode='bilinear', align_corners=True) * args.width
            ...
            if global_step % 100 == 0:
                ...
                if args.dataset == "kitti":
                    # TODO: fix bug in save_kitti_metrics, flip and swap not considered.
                    ...
                    disps_np = sample["left_stereo_disp_est"].data.cpu().numpy()[:,0]
                    save_kitti_metrics(logger, "train/stereo", disps_np, sample["left_fn"][:len(disps_np)], global_step)
...
def test():
    ...
    test_dataset = DistillDataset(args.data_path, args.test_list, args, args.dataset, 'test')
    ...
    for istep, sample in enumerate(test_dataloader):
        ...
        disp_est, _, _ = model(to_cuda_vars(sample))
